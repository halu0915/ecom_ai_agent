...
# Handle Playwright setup on Streamlit Cloud automatically
import subprocess
try:
    # Try running a playwright command to see if it's there
    subprocess.run(["playwright", "--version"], capture_output=True)
except FileNotFoundError:
    # On Streamlit Cloud, the path might be different or needed
    pass

# Try to force install chromium once at startup
if 'playwright_ready' not in st.session_state:
    with st.spinner("首次啟動：正在配置瀏覽器環境 (約需 30 秒)..."):
        try:
            subprocess.run([sys.executable, "-m", "playwright", "install", "chromium"], check=True)
            st.session_state['playwright_ready'] = True
        except Exception as e:
            st.error(f"瀏覽器環境安裝失敗：{e}")

from app.db import get_conn
from app.query_engine import run_query, build_insights
from app.reports import query_result_markdown, write_markdown

DB_PATH = ROOT / 'output' / 'db' / 'products.db'

st.set_page_config(page_title='AI 企業名錄 & 電商拓客系統', layout='wide', page_icon='🚀')
st.title('🚀 AI 企業名錄 & 電商拓客系統')

with st.sidebar:
    st.header('系統資訊')
    st.info('本系統整合了 IYP、Web66 及 Shopee 即時數據抓取與 AI 數據分析功能。')
    st.markdown('---')
    st.write('**功能狀態**')
    st.success('✅ 即時同步引擎：運作中')
    st.success('✅ Excel 樣式引擎：已搭載')
